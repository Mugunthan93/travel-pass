...
export class BusResultState {
    ...
    static getPolicies(state: busresult): any[] {
        ...
        let policy: any[] = arr.map(
            (el) => {
                return _.toPairs(el);
            }
        ).flat();
        let flatPolicy : string[][] = policy.map(el => el.map(e => _.startCase(e)));
        return flatPolicy;
    }
    ...
    getBusResponse(states: StateContext<busresult>, action: BusResponse) {
        ...
        let response2: busResponse[] = response.map(
            (res: busResponse) => {
                let el = res;
                if (el.fare.includes(',')) {
                    el.fare = Math.max(...el.fare.split(',').map(Number)) as unknown as string;
                }
                return el;
            }
        );
    ...
    getSeatLayout(states: StateContext<busresult>, action: SeatLayout) {
        ...
        let payload: seatPayload = {
            ...
            routeScheduleId: parseInt(action.busDetail.routeScheduleId),
    ...
    addSelectedSeat(states: StateContext<busresult>, action: AddSelectedSeat) {
        ...
        let currentselected: seat[] = Object.assign([], states.getState().selectedSeat);
    ...
    removeSelectedSeat(states: StateContext<busresult>, action: RemoveSelectedSeat) {

        let currentselected: seat[] = Object.assign([], states.getState().selectedSeat);

        let filteredSelection : seat[] = currentselected.filter(el => !(el.id == action.seat.id));
        states.patchState({
    ...
    selectedLowerSeat(states: StateContext<busresult>, action: SelectedLowerSeat) {

        let lowerseats: seat[][] = states.getState().lowerSeat;
        let selected : seat[][] = lowerseats.map(
            (el : seat[]) => {
                return el.map(
                    (e: seat) => {
    ...
    selectedUpperSeat(states: StateContext<busresult>, action: SelectedUpperSeat) {

        let upperseats: seat[][] = states.getState().upperSeat;
        let selected: seat[][] = upperseats.map(
            (el: seat[]) => {
                return el.map(
                    (e: seat) => {
                        if (_.isEqual(action.seat, e)) {
    ...
    getBusBook(states: StateContext<busresult>) {
        ...
        states.dispatch(new GetBookDetail(
    ...
    matrix(seats : seat[], index: number) : seat [][] {
        ...
        let row: number[] = seats.map(el => el.zIndex == index ? el.row : undefined);
        let column: number[] = seats.map(el => el.zIndex == index ? el.column : undefined);
        ...
        if (Number.isNaN(rowMax) && Number.isNaN(columnMax)) {
            ...
            let newseats : seat[][] = new Array(rowMax).fill(null).map(() => new Array(columnMax).fill(null));
            seats.forEach(
                (el) => {
                    if (el.zIndex == index) {
