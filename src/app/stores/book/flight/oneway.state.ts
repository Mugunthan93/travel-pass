...
export class OneWayBookState{
        ...
        return loading$
            .pipe(
                ...
                flatMap((response) => {
                    ...
                    let type = this.store.selectSnapshot(OneWaySearchState.getTripType);
                    return states.dispatch([
                        new SetFare(fareQuote.Fare),
                        new GetPLB(fareQuote,type),
                        new SetTaxable(),
                        new SetGST(fareQuote,type)
                    ]).pipe(
                        flatMap(() => {
                            return of(response);
                        })
    ...
    sendRequestPayload(states : StateContext<onewayBook>,action : FlightOneWaySendRequest | BookOneWayTicket) {
        ...
        let gst : GST = this.store.selectSnapshot(FLightBookState.getGST);
    ...
    onewaybookData(data: flightResult): bookObj {
        ...
        let pl = this.store.selectSnapshot(FLightBookState.getPLB)[0];
        let basefare = data.Fare.BaseFare;
